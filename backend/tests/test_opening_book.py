...
class TestOpeningBookAPI:
    def test_get_book_analysis(self, client, db):
        make_game(db, id="g1", platform_id="g1")
        make_move_analysis(db, game_id="g1", move_number=0, game_phase="opening")

        resp = client.get("/api/opening-book/g1")
        assert resp.status_code == 200
        data = resp.json()
        assert data["game_id"] == "g1"

    def test_get_book_analysis_not_found(self, client):
        resp = client.get("/api/opening-book/nonexistent")
        assert resp.status_code == 200
        data = resp.json()
        assert "error" in data

    def test_get_repertoire(self, client, db):
        make_game(db, id="g1", platform_id="g1")
        make_move_analysis(db, game_id="g1", move_number=0, game_phase="opening")

        resp = client.get("/api/opening-book")
        assert resp.status_code == 200
